...
def make_network(folder, **filters):
	...
	**filters :
	...
	if filters.get('retweets'):
		new_retweet_parser = update_retweet_parser(filters.get('keywords'), filters.get('senders_rt'), filters.get('receivers_rt'), filters.get('languages'))
		exec(new_retweet_parser, globals())
	
	if filters.get('mentions'):
		pass 
	
	# Second, parse line by file
	edges = []
	for file in files:
...
el = make_network(folder, retweets = True, dates = ['20200831', '20200901'], languages = ['en'], keywords = ['shaq'])
